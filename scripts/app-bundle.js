...
define('api/web-api-users',["require", "exports", "aurelia-fetch-client", "aurelia-framework"], function (require, exports, aurelia_fetch_client_1, aurelia_framework_1) {
    ...
    var data_users_X_profile = 'src/api/dummy-user-all.json';
        ...
        WebAPIUsers.prototype.getUserDetails = function (id) {
            ...
            return new Promise(function (resolve) {
                setTimeout(function () {
                    ...
                    var found = _this.http.fetch(data_users_X_profile)
    ...
    var UserSelected = (function () {
        ...
        UserSelected.prototype.activate = function (params, routeConfig) {
            ...
            return this.api.getUserDetails(params.id).then(function (user) {
                ...
                _this.user = user;
                _this.profile = {
                    regionId: _this.user['region'] ? _this.user['region'].id : null,
                    hubId: _this.user['hub'] ? _this.user['hub'].id : null,
                    segmentId: _this.user['segment'] ? _this.user['segment'].id : null,
                    entityId: _this.user['entity'] ? _this.user['entity'].id : null,
                    languages: _this.user['languages'] ? [] : null,
                    passports: _this.user['passports'] ? [] : null,
                    visas: _this.user['visas'] ? [] : null,
                    trainings: _this.user['trainings'] ? [] : null
                };
                var i = 0;
                if (_this.user['languages'].length) {
                    for (i = 0; i < _this.user['languages'].length; i++) {
                        var tmpLevel = !_this.user['languages'][i].proficiency ? null : _this.user['languages'][i].proficiency.value;
                        _this.profile['languages'].push({ languageId: _this.user['languages'][i].language.id, proficiencyValue: tmpLevel });
                    }
                }
                if (_this.user['passports'].length) {
                    for (i = 0; i < _this.user['passports'].length; i++) {
                        var tmpCountry = !_this.user['passports'][i].country ? null : _this.user['passports'][i].country.id;
                        var tmpType = !_this.user['passports'][i].type ? null : _this.user['passports'][i].type.value;
                        _this.profile['passports'].push({
                            countryId: tmpCountry,
                            number: _this.user['passports'][i].number,
                            typeValue: tmpType,
                            expiresOn: _this.user['passports'][i].expiresOn
                        });
                    }
                }
                if (_this.user['visas'].length) {
                    for (i = 0; i < _this.user['visas'].length; i++) {
                        var tmpCountry = !_this.user['visas'][i].country ? null : _this.user['visas'][i].country.id;
                        var tmpType = !_this.user['visas'][i].type ? null : _this.user['visas'][i].type.value;
                        _this.profile['visas'].push({
                            countryId: tmpCountry,
                            typeValue: tmpType,
                            multipleEntry: _this.user['visas'][i].multipleEntry,
                            expiresOn: _this.user['visas'][i].expiresOn
                        });
                    }
                }
                if (_this.user['trainings'].length) {
                    for (i = 0; i < _this.user['trainings'].length; i++) {
                        var tmpTraining = !_this.user['trainings'][i].training ? null : _this.user['trainings'][i].training.id;
                        _this.profile['trainings'].push({
                            trainingId: tmpTraining,
                            expiresOn: _this.user['trainings'][i].expiresOn
                        });
                    }
                }
